...
mod3 <- mboost(value ~ bbs(t, by = dim_y, #knots = 3,
                           boundary.knots = c(0,1), df = 5,
                           constraint = "symmetric",
                           cyclic = TRUE) %+% 
                 bbs(t, by = dim_x, boundary.knots = c(0,1), df = 5, 
                     constraint = "antisymmetric",  #knots = 3,
                     cyclic = TRUE
                     ), offset = 0,
               data = d2)
pred3 <- predict(mod3)#, newdata = newdat)
lines(head(pred3, nrow(d)), tail(pred3, nrow(d)), lty = "dotted", col ="darkred", t = "b")
...
pred3 <- predict(mod3, newdata = newdat)
lines(head(pred3, length(t_grid)), tail(pred3, length(t_grid)), col ="darkred", t = "l")
